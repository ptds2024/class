...
```
* See <https://github.com/ptds2024/bacteria> for a minimal example.


---
# `shinyuieditor`

`shinyuieditor` is an R package that allows you to visually design Shiny user interfaces directly from RStudio.  
It simplifies the process of building UIs, especially for developers who prefer graphical interfaces over code.
...
# Installing `shinyuieditor`

You can install `shinyuieditor` using the following command:
...
Once installed, you can start the visual editor by launching the add-in from RStudio:

```r
library(shinyuieditor)
shinyuieditor::launch_editor(app_loc = getwd())
```

---

# Demo: Building a UI with `shinyuieditor`

Example workflow:

1. Open **shinyuieditor** from RStudio.
2. Drag-and-drop elements like text inputs, sliders, and plots onto the canvas.
3. Customize each component (e.g., set the number of bins for a histogram).
4. Preview the UI live within the editor.
5. Export the Shiny UI code.

Resulting code in `app.R` in the chosen location.

---
# `shinydashboard`

**shinydashboard** is an R package that simplifies the creation of dashboards in Shiny. It provides a structured layout with customizable components, making it easier to organize data and UI elements.

- **Structured layout**: Includes sidebar, header, and body sections for intuitive organization.
- **Customizable themes**: Choose colors, icons, and design elements for each part of the dashboard.
- **Responsive design**: Optimized for various screen sizes, allowing dashboards to be viewed on different devices.
